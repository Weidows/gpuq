...
```bash
uv sync                        # create .venv from uv.lock (core + dev + host extras)
uv run gpuq serve              # start the server (http://127.0.0.1:8765)
uv run gpuq submit --project demo --user you --vram 2000 -- python main.py
uv run python tests/smoke.py   # smoke test; GPUQ_SMOKE_FAKE_GPU=1 when no GPU
```

Without uv you can use plain pip: `pip install -e ".[dev,host]" && gpuq serve`.

### Regenerating the Pages demo mock data

The live demo at `https://weidows.github.io/gpuq/` runs against `docs/mock-data.js`,
a deterministic snapshot generated by `scripts/gen_mock_data.py` (then
time-aligned to page load by `docs/mock-api.js`):

```bash
uv run python scripts/gen_mock_data.py                      # -> docs/mock-data.js (seed 42, 1h)
uv run python scripts/gen_mock_data.py --seed 7 --hours 2   # different scene
uv run python scripts/gen_mock_data.py --json-out /tmp/mock # + raw JSON + per-task .log files
```

Commit the regenerated `docs/mock-data.js`; the Pages workflow redeploys automatically.
